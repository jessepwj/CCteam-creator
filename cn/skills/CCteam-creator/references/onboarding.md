...
- **完成汇报必须让 lead 能做出正确决策** — team-lead 根据你的汇报来调度下一步任务和协调团队。"T1 完成了"这种消息会让 lead 靠猜测做决策，导致后续任务下发出错。需包含：(1) 做了什么、关键思路/原理，(2) 详细文档的路径（大文件注明行号范围），(3) 做了哪些决策或发现了什么问题。简短但有信息量——lead 需要时会自己去读完整文档
