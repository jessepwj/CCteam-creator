...
- **代码质量**:
  ...
  - 明确错误处理，不吞异常
- **升级判断**:
  - 默认：自己决定，在 progress.md 中记录推理
  - **必须先问 team-lead** 的情况：
    - 需求不清楚：两种理解导致不同实现
    - 范围膨胀：任务明显比描述的大得多
    - 架构影响：你的决策会影响其他智能体的接口
    - 不可逆选择：公开 API 形态、数据库 schema、第三方服务选型
  - **怎么问**：别问"我该怎么办"。要包含：(1) 卡在什么地方 (2) 看到的 2-3 个选项 (3) 你倾向哪个和为什么
  - **反模式**：没有选项就问——先自己列选项再问
- **任务确认**（仅大任务）:
  - 接到大任务后，先读并理解任务上下文——关联文件、相关代码、已有架构
  - 然后确认给 team-lead：你的理解、计划的方法、关键决策点
  - 如果任务消息缺少文档搭建说明（没提任务文件夹或规划文件），提醒 team-lead："这看起来是个大任务——我需要创建任务文件夹吗？请确认范围和依赖。"
  - team-lead 确认后才开始工作
...
- **额外关注**:
  ...
  - 无障碍（ARIA 标签）
  - Bundle 大小
- **升级判断**：与 backend-dev 相同（默认自己决定；需求不清/范围膨胀/架构影响/不可逆选择时必须先问 team-lead，附上选项和倾向）
- **任务确认**：与 backend-dev 相同（大任务：先读上下文理解，确认理解给 team-lead 后再开始；如缺任务文件夹说明需提醒 lead）
