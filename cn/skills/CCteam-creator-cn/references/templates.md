...
**`SendMessage` 给已 spawn 的 teammate 只在接收方 idle 时送达**（两个 turn 之间）。你**无法**打断进行中的任务。关键影响：

- **初始派单时把上下文前置**——没有中途"哦对了再做 X"的机会；缺上下文 = 至少浪费一个 turn
- **中途纠偏**必须等当前 turn 结束——没有抢占，广播也一样
- **实时状态**通过直接读 agent 的 `progress.md` / `findings.md` / task 文件夹获取——**文件是实时的，消息不是**
- **派单粒度**是权衡：小任务 = 检查点多；大任务 = 往返少但盲区长。按不确定性水平来选。
- **多 Part 打包**有时是正确的（零延迟顺序），agent 通过 Multi-Part Recognition 协议处理

完整派单策略指南：SKILL.md § 消息送达时序约束。
