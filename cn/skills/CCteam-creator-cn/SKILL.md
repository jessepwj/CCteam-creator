...
description: >
  ...
  /CCteam-creator-cn with a project name, (4) user wants to organize a multi-phase project
...
**机制**（实验验证，别处没有文档化）：`SendMessage` 发给已 spawn 的 teammate **不会在 turn 中途送达**。接收方**只有在 idle 时（两个 turn 之间）**才会收到新消息。你**无法**在 agent 正在执行任务的过程中打断它——消息会排队，直到 agent 完成当前 turn 并回复之后才被取出。

**这是 spawn/消息系统的硬约束，不是纪律规则。** team-lead 必须围绕它来规划派单。

**对派单策略的影响**：

1. **初始消息里必须把一切前置**。既然你无法中途追加"哦对了再做个 X"，派单消息必须包含所有上下文、验收标准、依赖文件路径、约束、期望输出格式。缺了任何一项 = agent 会在错误假设上烧掉一整个 turn，你才有机会纠正。

2. **你无法中途纠正正在运行的 agent**。如果优先级变了或你发现了错误，**必须等它当前 turn 结束**。纠正消息会排队，在它下次 idle 时送达。派单时就要假设没有打断窗口。

3. **派单粒度是"速度 vs 控制"的权衡**：
   - **小任务** → 检查点多、纠偏快、SendMessage 开销大
   - **大任务** → 往返少、盲区长、方向错了难调整
   - **经验法则**：高不确定性工作（探索性、新领域、规格模糊）→ 小任务。低不确定性工作（规格清晰、例行、熟悉模式）→ 大任务。

4. **紧急广播并不紧急**。`SendMessage(to: "*", …)` 发停一切的信号，仍然只会按每个接收方的下次 idle 逐个送达。正在 turn 中的 agent 会跑完当前 turn。**没有抢占**。

5. **"进展怎么样？"式 ping 在 turn 中途不起作用**。你无法通过消息窥视进行中的工作。取而代之——直接读 agent 的 `progress.md` / `findings.md` / task 文件夹——**文件是实时的，消息不是**。这是最重要的操作推论：当你需要知道某个 agent **此刻**在做什么时，读它的文件，不要发消息给它。

6. **多 Part 打包有时是正确选择**。当某项工作的多个部分必须顺序执行、且中间不能有往返延迟（例如"实现 A → 立即跑 CI → 汇报全部结果"），就把它们打包到一条消息里，靠 agent 的 Multi-Part Recognition 协议（见 onboarding.md）逐条识别和执行。打包是在"检查点粒度"和"零延迟顺序"之间做取舍，两个方向都有成本，按任务性质选。

**推论——实时状态靠文件系统，不靠消息**：消息在 turn 边界上传递意图和决定；文件在任意时刻传递连续状态。当你需要一个 agent 的当前真相时，答案永远在它的文件里——`progress.md` 看"刚刚发生了什么"、`findings.md` 看"学到了什么"、`task_plan.md` 看"计划是什么"。消息传请求和响应，不传状态。
