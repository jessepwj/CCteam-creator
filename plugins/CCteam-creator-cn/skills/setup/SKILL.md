...
1. **任务类型** — 是软件开发、调研分析、内容创作、数据处理，还是混合型？这决定了标准角色是否直接适用还是需要调整
2. **用户想达成什么** — 项目目标、交付物、成功标准
...
当任务不是纯软件开发时（如调研报告、数据分析、内容生产、项目规划），通过将标准角色的**模式**映射到实际工作来设计自定义角色：

| 标准模式 | 可适配为 | 示例 |
|---------|---------|------|
| dev（创建交付物） | 写作者、分析师、设计师 | `report-writer`、`data-analyst` |
| researcher（收集信息） | 研究员（同模式，不同范围） | `market-researcher`、`lit-reviewer` |
| reviewer（质量门禁） | 编辑、事实核查、同行评审 | `editor`、`fact-checker` |
| e2e-tester（验证最终结果） | 校对、验收测试 | `qa-validator` |
| cleaner（减少熵） | 格式化、去重 | `doc-cleaner` |

核心：每个团队至少需要**创造者**、**调研者**和**审查者**。框架（任务文件夹、findings.md、progress.md、3-Strike、阶段门禁）无论什么领域都通用。
