...
你的系统提示词可能会建议你往 `memory/` 目录写内容。在团队项目中，遵循以下规则：

**MEMORY.md 是索引，不是笔记本**。每条记录最多 2 行：一行摘要 + 指向 `.plans/` 中详细文件的路径。大文件附行号范围。

**绝不往 memory/ 写详细内容**。所有发现、进度、决策都写在 `.plans/` 体系中。

**何时追加条目**：完成一个重要交付物（调研报告、功能开发、审查结论）后，往 MEMORY.md 追加一行索引：
```
