...
Team Pilot 基于两个优秀的开源项目构建：

| 项目 | Stars | 我们学到了什么 |
|------|-------|--------------|
| [**planning-with-files**](https://github.com/OthmanAdi/planning-with-files) | 16,000+ | Manus 风格的持久化 Markdown 规划 — 三文件模式（task_plan.md / findings.md / progress.md）能在上下文压缩后存活。"上下文窗口 = 内存，文件系统 = 磁盘" 的哲学是 Team Pilot 状态持久化的骨架。源自 [Manus $20 亿收购案](https://github.com/OthmanAdi/planning-with-files)背后的工作流模式。 |
...
Team Pilot 将他们的理念延伸到**多智能体协调**领域 — 编排多个专业智能体并行工作，配以结构化沟通和文件驱动的进度追踪。

---

Team Pilot 帮助你在 Claude Code 中搭建和管理并行 AI 智能体团队。不再是单个 AI 助手，而是协调多个专业智能体 — 开发者、研究员、测试员、审查员 — 共同协作完成项目。

## 它做什么

当你调用 `/team-project-setup` 时，Team Pilot 会：
...
Agent Teams 是 Claude Code 的实验性功能。通过设置环境变量来启用：
...
这会将 Team Pilot 作为 Claude Code 插件安装。中英文版本的 skill 都包含在内。
...
不是每个项目都需要全部角色。Team Pilot 会根据你的需求推荐合适的组合。
...
team-pilot/
  .claude-plugin/
    plugin.json                       -- 插件元数据
  skills/
    team-project-setup/               -- 中文版
      SKILL.md
      references/
        roles.md
        onboarding.md
        templates.md
    team-project-setup-en/            -- 英文版
      SKILL.md
      references/
        roles.md
        onboarding.md
