...
- **Contract-First** (mandatory when frontend/backend are implemented by different agents): define API field table in `docs/api-contracts.md` (name, type, unit, optionality) BEFORE either side codes; both sides copy from contract, never invent locally; ambiguity-prone fields MUST carry unit annotations
- **Doc-Code Sync** (mandatory): API changes → update `docs/api-contracts.md`; architecture changes → update `docs/architecture.md`. Undocumented APIs don't exist for other agents
- **Env side effects in completion report**: state `none` / `done by me` / `needs team-lead action` for restart/migration/cache/config reload needs. Silent omission breaks next-agent verification
...
- **Contract-First / Doc-Code Sync / CI Gate / Env side effects**: same as backend-dev (read field table from `docs/api-contracts.md` before writing `types/*.ts`; frontend-only changes usually declare `none` for env side effects)
...
- **Explore-Then-Codify for unfamiliar UI** (recommended): before writing a new `.spec` for UI you haven't interacted with, run one interactive MCP exploration pass (`browser_navigate` / `browser_click` / `browser_snapshot`), record real selectors and async timings to `test-<scope>/exploration.md`, then copy selectors into the spec. No guessed selectors. Regression specs on already-exercised flows are exempt
- **Quality Standards**:
...
- **Anti-Phantom Finding Protocol**: grep-verify still-open findings from prior reviews before writing new ones; every new finding needs current-commit evidence; repo-wide Glob before `[NOT-FOUND]`; recurring phantom classes → `[AUTOMATE]` → custodian. Full protocol in onboarding.md
